...
export default function CheckoutAddressPage() {
  ...
  // Order params passed from Items page (step 1)
  const itemsOrderTotal = searchParams.get('orderTotal');
  const itemsShippingAmount = searchParams.get('shippingAmount');
  const itemsServiceCode = searchParams.get('serviceCode');
  const itemsDiscountAmount = searchParams.get('discountAmount');
  const itemsSubtotal = searchParams.get('subtotal');
  const itemsCoupon = searchParams.get('coupon');
  ...
  async function handleSaveAddresses() {
    ...
    try {
      ...
      // Build payment URL with all order params from items step
      const finalBillingId = nextBillingId;
      const finalShippingId = sameAsBilling ? nextBillingId : nextShippingId;
      const params = new URLSearchParams({
        ...(itemsOrderTotal && { orderTotal: itemsOrderTotal }),
        ...(itemsShippingAmount && { shippingAmount: itemsShippingAmount }),
        ...(itemsServiceCode && { serviceCode: itemsServiceCode }),
        ...(itemsDiscountAmount && { discountAmount: itemsDiscountAmount }),
        ...(itemsSubtotal && { subtotal: itemsSubtotal }),
        ...(itemsCoupon && { coupon: itemsCoupon }),
        billing: finalBillingId,
        shipping: finalShippingId,
      });
      setTimeout(() => {
        router.push(`/landing/checkout/payment?${params.toString()}`);
      }, 500);
    } catch (err: any) {
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {redirecting ? (
          ...
          <>
            ...
            <div className="mb-8">
              <div className="flex items-center max-w-sm">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-xs mt-1 font-semibold text-primary">Address</span>
                </div>
                <div className="flex-1 h-0.5 bg-gray-200 mx-3 mb-5" />
                <div className="flex flex-col items-center">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold bg-white border-2 border-gray-200 text-gray-300 cursor-not-allowed">
                    3
                  </div>
            ...
            <div className="mb-6">
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Shipping & Billing Address</h1>
            ...
            <div>
              <Card className="border-gray-200">
                <CardContent className="p-6">
                  ...
                  {addrSuccess && <div className="text-sm text-green-600 mt-4 text-center">✓ Addresses saved! Redirecting to payment…</div>}
                </CardContent>
              </Card>
            </div>
          </>
