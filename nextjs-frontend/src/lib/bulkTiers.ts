...
// REGULAR listed price. This is intentionally SEPARATE from wholesale
// (account-based) pricing — do not merge the two. The bands and percentages are
// uniform across the whole catalog; only the base price varies per product.
//
//   qty 1-3  -> 0%   (regular listed price)
//   qty 4-5  -> 10% off
//   qty 6-9  -> 16% off
//   qty 10+  -> 25% off
export type BulkTier = { min: number; max: number | null; discount: number };
...
export function bulkTierForQty(qty: number): BulkTier {
...
export function bulkUnitPrice(regularPrice: number, qty: number): number {
...
export function isRetailPricing(pricingCustomerType?: string | null): boolean {
  return !pricingCustomerType || pricingCustomerType === "B2C";
}

// Human label for a tier row, e.g. "4-5" or "10+".
export function bulkTierRangeLabel(t: BulkTier): string {
